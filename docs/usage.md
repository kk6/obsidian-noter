...
Obsidian Noterはデフォルトで外部APIを使わないルールベースの要約を生成します。このモードでは：
...
このモードは外部サービスへの依存なしで動作しますが、要約の質はシンプルなルールに基づいています。

### API連携モード (オプション)

APIキーを設定している場合、より高度な要約が生成されます：

- 内容の理解に基づいた要約
- 文脈を考慮したキーポイントの抽出
- より自然な文章構成

この機能を使用するには、[インストールガイド](installation.md)のAPI連携セクションを参照してください。
...
A: デフォルトのルールベース要約は、重要な文や段落を抽出する基本的なアルゴリズムを使用しています。API連携モードを有効にすると、より高品質な要約が得られますが、基本機能だけでも十分実用的です。

### Q: オフラインで使用できますか？

A: 記事抽出にはインターネット接続が必要ですが、要約処理自体は外部APIに依存せずローカルで実行されます。ただし、URLからの記事抽出にはインターネット接続が必要です。

### Q: 要約の品質に満足できない場合はどうすればよいですか？

A: より具体的な指示を与えてみてください。例えば「この記事をより詳細に要約してObsidianに保存してください」など。また、オプションとしてAPI連携モードを有効にすることでより高品質な要約が得られます。
